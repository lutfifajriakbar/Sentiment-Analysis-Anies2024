# -*- coding: utf-8 -*-
"""Skripsi Analisis Sentimen Twitter Anies Baswedan 90:10_Lutfi Fajri Akbar_201910225014.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1rdkqPY-l9j6pB2x_FZshHlIAb6YbAQIc

# **Penerapan Analysis Sentimen Twittter pada Keyword "Elektabilitas Anies" Menggunakan Algoritma Naive Bayes dan SVM**

# **Perbandingan Data Latih dan Data Uji 90:10**
...
def normalization(tokens, slang_words):
    normalized_tokens = []
    for token in tokens:
...
df['review'] = df['review'].apply(lambda x: normalization(x, slang_words))
...
x_train, x_test, y_train, y_test = train_test_split(review, label, test_size=0.1)

len(x_train)

len(x_test)

# Jumlah contoh sampel yang ingin ditampilkan
num_samples_to_display = 5

# Menampilkan contoh sampel data dari data training
for i in range(num_samples_to_display):
    print("Contoh Sampel ke-", i+1)
    print("Data Fitur (x_train):", x_train[i])
    print("Label (y_train):", y_train[i])
    print("\n")
...
np.savez('sentiment_data_90_10.npz', x_train=x_train, x_test=x_test, y_train=y_train, y_test=y_test)

import numpy as np

# Membaca file .npz
data = np.load('sentiment_data_90_10.npz', allow_pickle=True)
...
"""## Uji Prediksi Model SVM"""

# Menginputkan data teks secara manual
input_text = input("Masukkan teks/kalimat yang ingin diprediksi: ")

# Mengubah data teks menjadi vektor TF-IDF menggunakan kamus yang sudah dipelajari sebelumnya
input_text_transformed = vectorizer.transform([input_text])

# Melakukan prediksi pada data teks yang diinputkan
predicted_label = svc_model.predict(input_text_transformed)
...
print("Teks/kalimat termasuk kelas: ", label)
...
"""# Visualisasi Polaritas Sentimen Masyarakat"""

import matplotlib.pyplot as plt

# Menampilkan hasil analisis sentimen dalam bentuk Pie Chart
positive_count = np.sum(y_predsvc == 1)
negative_count = np.sum(y_predsvc == 0)
labels = ['Positive', 'Negative']
sizes = [positive_count, negative_count]
colors = ['#33FF99', '#FF3333']

plt.pie(sizes, labels=labels, colors=colors, autopct='%1.1f%%', startangle=90)
plt.axis('equal')
plt.title('Polaritas Sentimen Masyarakat')
plt.show()
